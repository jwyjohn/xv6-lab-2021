...
object *number_to_string_proc(object *arguments)
{
  ...
  char buf[100];
  int i, neg, base = 10, sgn = 1, xx = (car(arguments))->data.fixnum.value;
  uint x;
...
object *close_input_port_proc(object *arguments)
{
  ...
  result = close(car(arguments)->data.input_port.stream);
  if (result == EOF)
  {
...
object *close_output_port_proc(object *arguments)
{
  ...
  result = close(car(arguments)->data.output_port.stream);
  if (result == EOF)
  {
...
object *make_input_port(FD stream)
{
  object *obj;
...
object *make_output_port(FD stream)
{
  object *obj;
...
void init(void)
{
  ...
  memset(bufptr, 0x00, sizeof(bufptr));
}
...
void eat_expected_string(FD in, char *str)
{
  int c;
...
void peek_expected_delimiter(FD in)
{
...
void write_pair(FD out, object *pair)
{
  object *car_obj;
  object *cdr_obj;
...
int main(void)
{
  ...
  printf(
        "  _______             __    _          \n"
        " /_  __(_)___  __  __/ /   (_)________ \n"
        "  / / / / __ \\/ / / / /   / / ___/ __ \\ \n"
        " / / / / / / / /_/ / /___/ (__  ) /_/ /\n"
        "/_/ /_/_/ /_/\\__, /_____/_/____/ .___/ \n"
        "            /____/            /_/      \n"
        );

  printf("*** Tiny Lisp v0.01 ***\n");
  ...
  while (1)
  {
    ...
    _write(stdout, eval(exp, the_global_environment));
    printf("\n");
  }
